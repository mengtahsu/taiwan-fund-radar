...
assert(marketQuotesFunctionSource.includes("CACHE_TTL_MS = 60_000"), "market quote function should cache upstream quotes for one minute");
assert(marketQuotesFunctionSource.includes('"Cache-Control": "no-store"'), "browser reloads should always consult the shared market cache");
assert(marketQuotesFunctionSource.includes("readDatabaseCache"), "market quote function should share its cache across edge instances");
assert(marketQuotesFunctionSource.includes("writeDatabaseCache"), "market quote function should persist successful market quotes");
assert(marketQuotesFunctionSource.includes("Promise.allSettled"), "market quote function should tolerate partial source failures");
assert(marketQuotesFunctionSource.includes("price === null || price <= 0"), "market quote function should reject invalid prices");
assert(marketQuoteCacheSqlSource.includes("enable row level security"), "market quote cache should enable RLS");
assert(marketQuoteCacheSqlSource.includes("revoke all on table public.market_quote_cache from anon, authenticated"), "browser clients must not access the market quote cache table directly");
...
assert(appSource.includes("MARKET_QUOTES_FUNCTION_URL"), "page reload should request the edge market quote endpoint");
assert(appSource.includes("mergedMarkets"), "live market quotes should merge with the committed fallback payload");
...
assert(workflowSource.includes('cron: "15 0-3,22-23 * * *"'), "04:00 update should start guarded attempts up to 5h45 early");
assert(workflowSource.includes('cron: "15 6-11 * * *"'), "12:00 update should start guarded attempts up to 5h45 early");
assert(workflowSource.includes('cron: "15 14-19 * * *"'), "20:00 update should start guarded attempts up to 5h45 early");
assert((workflowSource.match(/timezone: ["']?Asia\/Taipei["']?/g) || []).length === 3, "scheduled updates should declare Taiwan time for all targets");
assert(workflowSource.includes("scripts/scheduled-update-gate.py"), "scheduled retries should use the freshness gate");
assert(scheduleGateSource.includes("EARLY_WINDOW_MINUTES = 345"), "early attempts should cover GitHub scheduler delays up to 5h45");
assert(scheduleGateSource.includes("target_is_complete"), "scheduled retries should not overwrite a completed target timestamp");
...
assert(workflowSource.includes("wait_for_scheduled_time"), "schedule waiting should not occupy the deployment concurrency group");
assert(workflowSource.includes("timeout-minutes: 350"), "the waiting job should support the earliest guarded attempt");
